...
PYPYLOG=jit-summary:../traces/~a-~a.trace ~a ~a/~a.rkt &>> ../timings-traces/~a-~a-~a-traces.rst
   ...
   (for ([b (in-list benchmarks)])
     (let* ([bash-script-name (if (equal? sys 'racket)
            ...
            [command (if (equal? sys 'racket)
                         ...
                         (if (equal? m 'new) "~/pycketland/pycket/pycket-c" "~/pycketland/old-pycket/pycket-c"))]
            [src-dir (if traces
	    	     	 "../src/for-traces"
	    	         (if warm 
                             "../src/with-warmup"

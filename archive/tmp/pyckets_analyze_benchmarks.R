...
bench$vm <- sapply(bench$vm, function (x)
  if (x=='RacketOLD') 'Racket' else paste0("",x))
...
# message(" << factor bench$vm")
# print(factor(bench$vm))
# message(" <<")

if ('Pycket' %in% bench$vm) {
  if (length(factor(bench$vm)) > 2) {
    message(" -- IF > 2 --")
    bench$vm <- factor(bench$vm, levels = c("Pycket", "PycketNEW"))
  } else {
    message(" -- ELSE --")
    bench$vm <- factor(bench$vm, levels = c("Pycket", "PycketNEW"))
  }
...
#reference.vm <-  if ('RacketOLD' %in% bench$vm) 'Racket' else 'Pycket'
reference.vm <- 'Pycket'
...
mn <- mapply(function(vm, n) {if (vm == "RacketOLD")  n else Inf }, bench.summary.graph$vm, bench.summary.graph$mean.norm)
